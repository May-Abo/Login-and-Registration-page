...
public class UserServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            UserWeb uw = di.logInValidation(username, password);
            System.out.println(uw);

            if (uw == null) {
                response.sendRedirect("Login.jsp?errorMsg=1");
            } else {
                request.getSession().setAttribute("userlog", uw);
                response.sendRedirect("Profile.jsp");
                UserLog ul = new UserLog(uw.getUserName(), request.getLocalAddr(), "Log In", "");
                di.addUserLogInfo(ul);
            }
